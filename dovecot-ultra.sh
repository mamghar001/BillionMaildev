...
# Ultra-fast entrypoint - chown is no-op since ownership is managed
# via the volume mount (UID 150 from host maps to vmail:mail)

id vmail || useradd -r -u 150 -g mail -d /var/vmail -s /sbin/nologin -c "Virtual Mail User" vmail

# Verify /var/vmail directory exists and is accessible
# (we trust the volume mount to set correct ownership)
chown vmail:mail /var/vmail 2>/dev/null
