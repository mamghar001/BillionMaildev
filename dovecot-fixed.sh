...
# Smart entrypoint - only fix ownership of files NOT already correct
# (vs BM default chown -R which walks every file recursively)

id vmail || useradd -r -u 150 -g mail -d /var/vmail -s /sbin/nologin -c "Virtual Mail User" vmail

# Smart chown: only touch files with wrong ownership
# This is ~1000x faster than chown -R when files are already correct
find /var/vmail \( ! -user 150 -o ! -group 8 \) -exec chown vmail:mail {} + 2>/dev/null
# Also fix the top dir itself
